-- Fix marketing automation migration by referencing salons table instead of profiles.salon_id
...
BEGIN
  ...
  LOOP
    ...
    AND EXTRACT(DOY FROM p.birth_date) = EXTRACT(DOY FROM (now() + INTERVAL '7 days'))
